...
use Carbon\Carbon;
use App\Models\{Course, Category, Review,Lesson};
...
if (!function_exists('course_data')) {
    function course_data($courses = array())
    {
        foreach ($courses as $key => $course) {
            ...
            $courses[$key]->outcomes = isset($course->outcomes) && ($decoded = json_decode($course->outcomes)) && !empty($decoded) ? $decoded : null;
            ...
            $courses[$key]->watch_time_key =  gmdate('H:i:s', $totalSeconds);
...
if (!function_exists('course_details_by_id')) {
    ...
    $watchHistory = Watch_history::where('course_id', $course_id)
        ...
        foreach ($response as $key => $resp) {
            $response[$key]->sections = sections($course_id,$user_id);
            ...
            $response[$key]->watch_time_key =get_total_duration_of_lesson_by_course_id($course_id);
            $response[$key]-> is_completed = $isCompleted;
...
function is_purchased($user_id = 0, $course_id = "")
{
    ...
    if ($user_id > 0) {
        if (check_enroll($course_id, $user_id) == 'valid') {
            return true;
        } else {
            return false;
        }
...
if (!function_exists('check_enroll')) {
    function check_enroll($course_id = "", $user_id = "")
    {
...
if (!function_exists('update_watch_history_manually')) {
    ...
    function update_watch_history_manually($lesson_id = "", $course_id = "", $user_id = "")
    {
        ...
        if (!empty($query)) {
            ...
            Watch_history::create($insert_data);
        }
